...
import { TableMenuTrigger } from '@common/text-editor/menubar/table-menu-trigger';
import { FontMenuTrigger } from '@common/text-editor/menubar/font-menu-trigger';

const MenubarRowClassName =
  'flex items-center justify-center px-4 h-42 text-muted border-b overflow-hidden';

export function PeopleBodyEditorMenubar({
  editor,
  size = 'md',
}: MenubarButtonProps) {
  ...
  return (
    <div className={clsx(extendedVisible ? 'h-84' : 'h-42')}>
      <div className={clsx(MenubarRowClassName, 'relative')}>
        <FormatMenuTrigger editor={editor} size={size} />
        <Divider />
        <FontMenuTrigger editor={editor} size={size} />
        <Divider />
...
function ExtendedButtons({editor, size = 'md'}: MenubarButtonProps) {
  return (
    <Fragment>
      ...
      <TableMenuTrigger editor={editor} size={size} />
